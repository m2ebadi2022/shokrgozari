...
Sub fill_list
	...
	ls_tamrin1_Roz=myFunc.get_daftar_khasteha_byShenase(Main.daftar_shenase)
	...
	For i=0 To ls_tamrin1_Roz.Size-1
		...
		p.LoadLayout("item_list_daftar_khasteha")
		
		et_khaste.Color=Colors.White
		...
		If(list_num.Get(2)<>"null")Then
			et_khaste.Text=list_num.Get(2)
		End If
		
		
		et_khaste.Tag=list_num.Get(0)
...
Private Sub et_khaste_TextChanged (Old As String, New As String)
	
	If(list_reload=0)Then
		Dim this_num_tamrin As EditText=Sender
		If(et_khaste.Text<>"" Or et_khaste.Text<>Null)Then
			myFunc.update_daftar_khasteha(this_num_tamrin.Tag,this_num_tamrin.Text,1)
		End If
...
Private Sub lbl_add_new_Click
	...
	Main.sql.ExecNonQuery2("INSERT INTO daftar_khasteha ('shenase') VALUES (?)", Array As Object(Main.daftar_shenase))
	Main.sql.Close
...
Private Sub lbl_num_LongClick
	...
	If Result = DialogResponse.POSITIVE Then
		myFunc.delet_daftar_khasteById(this_item.Tag)
		fill_list
	End If
